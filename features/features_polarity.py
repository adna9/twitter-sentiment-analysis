...
def calculateFeatures(message,process_message,tokens,process_tokens,pos,slangDictionary,lexicons,mpqa_lexicons,pos_bigrams,pos_trigrams,pos_bigrams_scores_negative,pos_bigrams_scores_positive,pos_trigrams_scores_negative,pos_trigrams_scores_positive,pos_tags_scores_negative,pos_tags_scores_positive,mpqaScores,negationList,clusters):
    ...
    x8 = numberOfPositiveEmoticons(tokens)
    #f.append(x)

    #the number of negative emoticons
    x9 = numberOfNegativeEmoticons(tokens)
    #f.append(x)
...
##    f.append(x9/float(s))
    ...
    for lexicon in lexicons:
        ...
        #max score of words
        x = maxOfScores(lexicon,process_tokens,pos,True)
        f.append(x)

        #min score of words
        x = minOfScores(lexicon,process_tokens,pos,True)
        f.append(x)
    ...
    x = hasNegationPrecedingLexicon(mpqa_lexicons[0],process_tokens,negationList)
    f.append(x)

    x = hasNegationPrecedingLexicon(mpqa_lexicons[1],process_tokens,negationList)
    f.append(x)

    x = hasNegationPrecedingLexicon(mpqa_lexicons[4],process_tokens,negationList)
    f.append(x)

    x = hasNegationPrecedingLexicon(mpqa_lexicons[5],process_tokens,negationList)
    f.append(x)
