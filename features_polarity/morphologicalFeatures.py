...
def hasUrlOrEllipsisAtEnd(last_token,last_tag):
    if(last_tag=="U" or last_token=="..." or last_token=="HTTP_LINK"):
        return 1
    else:
        return 0
...
def senti_score(tokens, lex) :

    return lex.score(tokens)

def numOfSlangs(tokens,slangDictionary):
    
    return len([token for token in tokens if slangDictionary.isSlang(token)])
